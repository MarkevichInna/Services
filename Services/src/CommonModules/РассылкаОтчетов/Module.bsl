...
Процедура ЗарегистрироватьДанныеДляАктуализацииСпискаРегламентныхЗаданий(Параметры) Экспорт
	ОбновлениеИнформационнойБазы.ОтметитьКОбработке(Параметры, ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.РассылкиОтчетов.ЛичныеРассылки"));
КонецПроцедуры
...
Процедура АктуализироватьСписокРегламентныхЗаданий(Параметры) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РегламентноеЗадание ИЗ Справочник.РассылкиОтчетов ГДЕ НЕ (Подготовлена И ВыполнятьПоРасписанию)";
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		Задание = РегламентныеЗадания.НайтиПоУникальномуИдентификатору(Выборка.РегламентноеЗадание);
	...
	ОбновлениеИнформационнойБазы.ОтметитьВыполнениеОбработки(ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.РассылкиОтчетов.ЛичныеРассылки"));
КонецПроцедуры
...
Процедура ВыполнитьРассылкуПоРасписанию(Рассылка) Экспорт
	// Запрос с отбором для переноса СрезаПервых из регистра сведений РегистрацияДействийПользователей
	// в регистр сведений РегистрацияДействийПользователей_СрезПервых 
	ДатаНастМесяца = ТекущаяДата()-2678400;
	
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	С_РегистрацияДействийПользователей.Период КАК Период,
	|	С_РегистрацияДействийПользователей.УНП,
	|	С_РегистрацияДействийПользователей.РегистрационныйНомерОсновнойПоставки,
	|	С_РегистрацияДействийПользователей.Действие,
	|	С_РегистрацияДействийПользователей.НазваниеОрганизации,
	|	С_РегистрацияДействийПользователей.Конфигурация
	|ИЗ
	|	РегистрСведений.С_РегистрацияДействийПользователей КАК С_РегистрацияДействийПользователей
	|ГДЕ
	|	НЕ С_РегистрацияДействийПользователей.УНП = """"
	|	И НЕ С_РегистрацияДействийПользователей.РегистрационныйНомерОсновнойПоставки = """"
	|	И НЕ С_РегистрацияДействийПользователей.НазваниеОрганизации = """"
	|   И НЕ С_РегистрацияДействийПользователей.Действие.Наименование = """"
	|   И НЕ С_РегистрацияДействийПользователей.Действие = """"
	|	И С_РегистрацияДействийПользователей.Действие ЕСТЬ НЕ NULL
	|
	|СГРУППИРОВАТЬ ПО
	|	С_РегистрацияДействийПользователей.Период,
	|	С_РегистрацияДействийПользователей.УНП,
	|	С_РегистрацияДействийПользователей.Конфигурация,
	|	С_РегистрацияДействийПользователей.Действие,
	|	С_РегистрацияДействийПользователей.НазваниеОрганизации,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		//Проверяем выбранную запись запроса на наличие дубля в регистре
		Регистр = РегистрыСведений.С_РегистрацияДействийПользователей_СрезПервых.СоздатьМенеджерЗаписи();
		Регистр.УНП = ВыборкаДетальныеЗаписи.УНП;
		Регистр.РегистрационныйНомерОсновнойПоставки = ВыборкаДетальныеЗаписи.РегистрационныйНомерОсновнойПоставки;
		Регистр.Действие = ВыборкаДетальныеЗаписи.Действие.Ссылка;
		Регистр.Прочитать();
		Если Регистр.Выбран()Тогда
			НайденоДублей = НайденоДублей + 1;
		Иначе
			//Записываем запись запроса в регистр сведений	
			Регистр = РегистрыСведений.С_РегистрацияДействийПользователей_СрезПервых.СоздатьМенеджерЗаписи();
			Регистр.УНП = ВыборкаДетальныеЗаписи.УНП;
			Регистр.РегистрационныйНомерОсновнойПоставки = ВыборкаДетальныеЗаписи.РегистрационныйНомерОсновнойПоставки;
			Регистр.Действие = ВыборкаДетальныеЗаписи.Действие.Ссылка;
			Регистр.НазваниеОрганизации = ВыборкаДетальныеЗаписи.НазваниеОрганизации;
			Регистр.Конфигурация = ВыборкаДетальныеЗаписи.Конфигурация;
			Регистр.РеквизитПериод = ВыборкаДетальныеЗаписи.Период;
			Регистр.Записать();
			Если ВыборкаДетальныеЗаписи.Период <= Дата(ДатаНастМесяца) Тогда
			Запись = РегистрыСведений.С_РегистрацияДействийПользователей.СоздатьМенеджерЗаписи();
	...
	// Запрос без отбора для удаления данных из регистра сведений РегистрацияДействийПользователей созданных ранее текущего месяца 
	...
	|	РегистрСведений.С_РегистрацияДействийПользователей КАК С_РегистрацияДействийПользователей
	//|ГДЕ
	//|	НЕ РегистрацияДействийПользователей.УНП = """"
	//|	И НЕ РегистрацияДействийПользователей.РегистрационныйНомерОсновнойПоставки = """"
	//|	И РегистрацияДействийПользователей.Действие.Ссылка ЕСТЬ НЕ NULL
	//|	И НЕ РегистрацияДействийПользователей.НазваниеОрганизации = """"
	//|
	|СГРУППИРОВАТЬ ПО
	|	С_РегистрацияДействийПользователей.Период,
	|	С_РегистрацияДействийПользователей.УНП,
