...
Функция ВерсииАдресныхОбъектов() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|ИЗ
		|	РегистрСведений.С_ВерсииОбъектовАдресногоКлассификатора КАК Версии
		|;
		|////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Регион
		|ГДЕ
		|	Регион.ТипАдресногоЭлемента = 1
		|	И 1 В (
		|		               ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде=Регион.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 2
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде=Регион.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 3
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде=Регион.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 4
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде=Регион.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 5
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде=Регион.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 6
		|	)
		|");
...
Процедура ЗагрузитьКлассификаторПоАдресномуОбъекту(Знач КодАдресногоОбъекта, Знач ПутьКДаннымНаСервере, Знач ЗагрузкаСВеб, Знач Версия) Экспорт
	...
	АдресныеСведения.Колонки.Добавить("SOATO");
	...
	ЗагрузитьАдресныеСведения_Локализация(КодАдресногоОбъекта, 
		ПутьКДаннымНаСервере + "CITY", АдресныеСведения, АльтернативныеНазвания);
		
	////// Загрузим улицы
	ЗагрузитьАдресныеСведения_Локализация(КодАдресногоОбъекта,
	    ПутьКДаннымНаСервере + "NNUL" + Постфикс, АдресныеСведения, АльтернативныеНазвания, 5);
	//		
	// нам не нужно выравнивать код, он правильный
	
	
	// Запись в одной транзакции с версией
	//НачатьТранзакцию();
	//Попытка
		НаборАдресныхСведений = РегистрыСведений.С_АдресныйКлассификатор.СоздатьНаборЗаписей();
		НаборАдресныхСведений.Отбор.КодАдресногоОбъектаВКоде.Установить( Число(КодАдресногоОбъекта) );
		НаборАдресныхСведений.Прочитать();
		НаборАдресныхСведений.Очистить();
...
Функция ЧислоЗаполненныхАдресныхОбъектов() Экспорт
	...
	Запрос = Новый Запрос("
		...
		|			1 
		|		ИЗ 
		|			РегистрСведений.С_АдресныйКлассификатор КАК Уровень2
		|		ГДЕ 
		...
		|			1 
		|		ИЗ 
		|			РегистрСведений.С_АдресныйКлассификатор КАК Уровень3
		|		ГДЕ 
		...
		|			1 
		|		ИЗ 
		|			РегистрСведений.С_АдресныйКлассификатор КАК Уровень4
		|		ГДЕ 
		...
		|			1 
		|		ИЗ 
		|			РегистрСведений.С_АдресныйКлассификатор КАК Уровень5
		|		ГДЕ 
		...
		|			1 
		|		ИЗ 
		|			РегистрСведений.С_АдресныйКлассификатор КАК Уровень6
		|		ГДЕ 
...
Функция КлассификаторЗагружен() Экспорт
	Запрос = Новый Запрос("ВЫБРАТЬ ПЕРВЫЕ 1
	                      |	С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде КАК КодАдресногоОбъектаВКоде
	                      |ИЗ
...
Функция ИнформацияПоАдресномуОбъекту(Знач КодАдресногоОбъекта) Экспорт
	...
	Иначе
		Классификатор = РегистрыСведений.С_АдресныйКлассификатор.КлассификаторСубъектовРФ();
		Если ТипЗнч(КодАдресногоОбъекта) = Тип("Строка") Тогда
			ТипЧисло = Новый ОписаниеТипов("Число");
			КодАдресногоОбъекта = ТипЧисло.ПривестиЗначение(КодАдресногоОбъекта);
		КонецЕсли;
		
		АдресныйОбъект = Классификатор.Найти(КодАдресногоОбъекта, "КлассификаторАдресныхОбъектовБеларуси");
...
Функция НайтиЗаписиПоИндексу(Индекс) Экспорт
	...
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента КАК ТипАдресногоЭлемента,
		|	С_АдресныйКлассификатор.Код                  КАК Код,
		|
		|	С_АдресныйКлассификатор.Наименование + "" "" + С_АдресныйКлассификатор.Сокращение КАК Наименование,
		|
		|	С_АдресныйКлассификатор.ПризнакАктуальности       КАК ПризнакАктуальности,
		|	С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде  КАК КодАдресногоОбъектаВКоде,
		|	С_АдресныйКлассификатор.КодРайонаВКоде            КАК КодРайонаВКоде,
		|	С_АдресныйКлассификатор.КодГородаВКоде            КАК КодГородаВКоде,
		|	С_АдресныйКлассификатор.КодНаселенногоПунктаВКоде КАК КодНаселенногоПунктаВКоде,
		|	С_АдресныйКлассификатор.КодУлицыВКоде             КАК КодУлицыВКоде
		|ПОМЕСТИТЬ 
		|	Адреса
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ГДЕ
		|	С_АдресныйКлассификатор.Индекс = &Индекс
		|ИНДЕКСИРОВАТЬ ПО 
...
Процедура ПоКодуАдресногоЭлементаВСтруктуруПолучитьЕгоКомпоненты(КодАдресногоЭлемента, Результат) Экспорт
	...
	Запрос.Текст = "
		...
		|	С_АдресныйКлассификатор.Код                  КАК Код,
		|
		|	ВЫБОР КОГДА С_АдресныйКлассификатор.ТипАдресногоЭлемента < 4 ТОГДА
		|	    С_АдресныйКлассификатор.Наименование + "" "" + С_АдресныйКлассификатор.Сокращение
		|	ИНАЧЕ
		|	    С_АдресныйКлассификатор.Сокращение  + "" "" + С_АдресныйКлассификатор.Наименование
		...
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ГДЕ
		|	С_АдресныйКлассификатор.Код = &КодАдресногоЭлемента
		|";
...
Функция СтруктураАдреса(КодАдресногоЭлемента, Дом = "", Корпус = "", Квартира = "") Экспорт
	...
	Запрос.Текст = "
		|ВЫБРАТЬ
		|	С_АдресныйКлассификатор.Индекс КАК Индекс
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ГДЕ
		|	С_АдресныйКлассификатор.Код = &Код
		|";
...
Функция ИндексАдреса(Знач НазваниеРегиона, Знач НазваниеРайона, Знач НазваниеГорода, Знач НазваниеНаселенногоПункта, Знач НазваниеУлицы, Знач НомерДома, Знач НомерКорпуса, РодительИндекса = Неопределено) Экспорт
	...
	РодительИндекса = С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса();
	...
	Запрос = Новый Запрос( " 
		...
		|	С_АдресныйКлассификатор.Код                       КАК Код,
		|	С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде  КАК КодАдресногоОбъектаВКоде,
		|	С_АдресныйКлассификатор.Наименование              КАК Наименование,
		|	С_АдресныйКлассификатор.Сокращение                КАК Сокращение,
		|	С_АдресныйКлассификатор.Индекс                    КАК Индекс,
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента      КАК ТипАдресногоЭлемента,
		|	С_АдресныйКлассификатор.КодРайонаВКоде            КАК КодРайонаВКоде,
		|	С_АдресныйКлассификатор.КодГородаВКоде            КАК КодГородаВКоде,
...
Функция ПроверитьСоответствиеАдресаКЛАДРу(Знач УказанныйИндекс = "", Знач НазваниеРегиона = "", Знач НазваниеРайона = "", Знач НазваниеГорода = "", Знач НазваниеНаселенногоПункта = "", Знач НазваниеУлицы = "", Знач НомерДома = "", Знач НомерКорпуса = "") Экспорт
	...
	Результат.Вставить("Регион",          С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	Результат.Вставить("Район",           С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	Результат.Вставить("Город",           С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	Результат.Вставить("НаселенныйПункт", С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	Результат.Вставить("Улица",           С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	Результат.Вставить("Дом",             С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса() );
	...
	Запрос = Новый Запрос("
		...
		|
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Адрес
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Улица
		|ПО
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК НаселенныйПункт
		|ПО
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Город
		|ПО
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Район
		|ПО
...
Функция АнализСоответствияАдресаКлассификатору(Знач ОбъектАдреса) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	ТаблицаКодов
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Дома
		|ПО	
...
Функция АдресныйЭлементЗагружен(Знач НазваниеРегиона, Знач НазваниеРайона = "", Знач НазваниеГорода = "", Знач НазваниеНаселенногоПункта = "", Знач НазваниеУлицы = "", Уровень = 1) Экспорт
	...
	Родитель = С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса();
	...
	Если Уровень=1 Тогда
		Запрос.Текст = "
			|               ВЫБРАТЬ ПЕРВЫЕ 1 Код ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ ТипАдресногоЭлемента=2 И КодАдресногоОбъектаВКоде=&КодАдресногоОбъектаВКоде
			|ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 Код ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ ТипАдресногоЭлемента=3 И КодАдресногоОбъектаВКоде=&КодАдресногоОбъектаВКоде
			|ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 Код ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ ТипАдресногоЭлемента=4 И КодАдресногоОбъектаВКоде=&КодАдресногоОбъектаВКоде
			|ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 Код ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ ТипАдресногоЭлемента=5 И КодАдресногоОбъектаВКоде=&КодАдресногоОбъектаВКоде
			|ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 Код ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ ТипАдресногоЭлемента=6 И КодАдресногоОбъектаВКоде=&КодАдресногоОбъектаВКоде
			|";
...
Функция ИмяИАдресноеСокращение(Знач СтрокаЭлемента, АдресноеСокращение) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|ИЗ 
		|	РегистрСведений.С_АдресныеСокращения КАК ВсеСокращения
		|ЛЕВОЕ СОЕДИНЕНИЕ (
		|	ВЫБРАТЬ ПЕРВЫЕ 1
		|		1 КАК ФлагСокращения
		|	ИЗ
		|		РегистрСведений.С_АдресныеСокращения
		|) КАК ПоискСокращения
		|ПО 
...
Функция НаименованиеРегионаПоКоду(КодРегиона) Экспорт
	...
	Запрос.Текст = "
		...
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = &ТипАдресногоЭлемента
		|	И С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде = &КодАдресногоОбъектаВКоде
		|";
	Результат = Запрос.Выполнить();
...
Функция КодРегионаПоНаименованию(Регион) Экспорт
	...
	Запрос = Новый Запрос("
		|ВЫБРАТЬ
		|	С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде КАК Код
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = &ТипАдресногоЭлемента
		|	И С_АдресныйКлассификатор.Наименование = &Наименование
		|");
	Запрос.УстановитьПараметр("ТипАдресногоЭлемента", 1);
...
Функция ВернутьСтруктуруАдресногоКлассификатораПоАдреснымЭлементам(Знач НазваниеРегиона, Знач НазваниеРайона, Знач НазваниеГорода, Знач НазваниеНаселенногоПункта, Знач НазваниеУлицы) Экспорт
	...
	РодительскийЭлемент = С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса();
	...
	Если РодительскийЭлемент = Неопределено Тогда
		Возврат С_АдресныйКлассификаторКлиентСервер.ПустаяСтруктураАдреса()
	Иначе
...
Функция РазрешенияБезопасностиПроверкиОбновления() Экспорт
	...
	//	АдресныйКлассификаторКлиентСервер_Локализация.ПутьКФайлуОписаниюДанныхКЛАДР()
	//);
	//
...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	//Обработчик.Процедура = "АдресныйКлассификатор_Локализация.ЗагрузитьАдресныеОбъектыПервогоУровня";
	//Обработчик.НачальноеЗаполнение = Истина;

	//Обработчик = Обработчики.Добавить();
	//Обработчик.Версия = "2.0.1.11";
	//Обработчик.Процедура = "АдресныйКлассификатор_Локализация.РазложитьАдресныеОбъектыПоЭлементам";
...
Процедура ЗагрузитьАдресныеОбъектыПервогоУровня() Экспорт
	...
	//КлассификаторАдресныхОбъектовXML = РегистрыСведений.АдресныйКлассификатор_Локализация.ПолучитьМакет("КлассификаторАдресныхОбъектовБеларуси").ПолучитьТекст();
	//   
	//КлассификаторТаблица = ОбщегоНазначения.ПрочитатьXMLВТаблицу(КлассификаторАдресныхОбъектовXML).Данные;
	//НаборЗаписей = РегистрыСведений.АдресныйКлассификатор_Локализация.СоздатьНаборЗаписей();
...
Процедура РазложитьАдресныеОбъектыПоЭлементам() Экспорт
	...
	// Обходим все записи регистра АдресныйКлассификатор_Локализация и добавляем неактуальные адреса в массив
	МассивНеактуальныхАдресов = Новый Массив;
	Выборка = РегистрыСведений.С_АдресныйКлассификатор.Выбрать(,"<Нет>");
	Пока Выборка.Следующий() Цикл
	...
	Для Каждого СтруктураАдреса Из МассивНеактуальныхАдресов Цикл
		...
		МенеджерЗаписи = РегистрыСведений.С_АдресныйКлассификатор.СоздатьМенеджерЗаписи();
...
Функция ЗагрузитьАдресныеСокращения(Знач ПутьКДаннымНаСервере, Знач Версия) Экспорт
	...
	АдресныеСокращения = РегистрыСведений.С_АдресныеСокращения;
...
Процедура ЗагрузкаАдресныхСведенийИзФайловКЛАДРВРегистрСведений(ПараметрыЗагрузки, АдресХранилища) Экспорт
	...
	Попытка
		...
		Для Каждого АдресныйОбъект Из ПараметрыЗагрузки.АдресныеОбъекты Цикл
			...
			//		АдресныйОбъект, РегистрыСведений.АдресныйКлассификатор_Локализация.НаименованиеРегионаПоКоду(АдресныйОбъект),
...
Процедура УстановитьВерсиюКлассификатора(Знач КодРегиона, Знач Версия = '00000000') Экспорт
	...
	Набор = РегистрыСведений.С_ВерсииОбъектовАдресногоКлассификатора.СоздатьНаборЗаписей();
	...
	Если Версия <> '00000000' Тогда
		...
		Данные.Наименование      = РегистрыСведений.С_АдресныйКлассификатор.НаименованиеРегионаПоКоду(КодСтрокой);
	КонецЕсли;
...
Процедура РазложитьЭлементыАдресаВоВременныеТаблицы(МенеджерВременныхТаблиц)
	...
	Запрос = Новый Запрос("
		|ВЫБРАТЬ
		|	Адреса.Код КАК Код,
		|	С_АдресныйКлассификатор.Код КАК КодРегиона,
		|	С_АдресныйКлассификатор.Наименование + "" "" + С_АдресныйКлассификатор.Сокращение КАК Наименование,
		|	С_АдресныйКлассификатор.ПризнакАктуальности КАК ПризнакАктуальности,
		...
		|	РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|ПО 
		|	Адреса.КодАдресногоОбъектаВКоде = С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = 1
		|;////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		|	Адреса.Код КАК Код,
		|	С_АдресныйКлассификатор.Код КАК КодРайона,
		|	С_АдресныйКлассификатор.Наименование + "" "" + С_АдресныйКлассификатор.Сокращение КАК Наименование,
		|	С_АдресныйКлассификатор.ПризнакАктуальности КАК ПризнакАктуальности,
		...
		|ПО 
		|	Адреса.КодАдресногоОбъектаВКоде = С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде
		|	И Адреса.КодРайонаВКоде = С_АдресныйКлассификатор.КодРайонаВКоде
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = 2
		|;////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		|	Адреса.Код КАК Код,
		|	С_АдресныйКлассификатор.Код КАК КодГорода,
		|	С_АдресныйКлассификатор.Наименование + "" "" + С_АдресныйКлассификатор.Сокращение КАК Наименование,
		|	С_АдресныйКлассификатор.ПризнакАктуальности КАК ПризнакАктуальности,
		...
		|	Адреса.КодАдресногоОбъектаВКоде = С_АдресныйКлассификатор.КодАдресногоОбъектаВКоде
		|	И Адреса.КодРайонаВКоде = С_АдресныйКлассификатор.КодРайонаВКоде
		|	И Адреса.КодГородаВКоде = С_АдресныйКлассификатор.КодГородаВКоде
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = 3
		|;////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		|	Адреса.Код КАК Код,
		|	С_АдресныйКлассификатор.Код КАК КодНаселенногоПункта,
		|	С_АдресныйКлассификатор.Сокращение + "" "" + С_АдресныйКлассификатор.Наименование КАК Наименование,
      	|	С_АдресныйКлассификатор.ПризнакАктуальности КАК ПризнакАктуальности,
		...
		|	И Адреса.КодРайонаВКоде = С_АдресныйКлассификатор.КодРайонаВКоде
		|	И Адреса.КодГородаВКоде = С_АдресныйКлассификатор.КодГородаВКоде
		|	И Адреса.КодНаселенногоПунктаВКоде = С_АдресныйКлассификатор.КодНаселенногоПунктаВКоде
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = 4
		|;////////////////////////////////////////////////////////////////////////////////
		|
		|ВЫБРАТЬ
		|	Адреса.Код КАК Код,
		|	С_АдресныйКлассификатор.Код КАК КодУлицы,
		|	С_АдресныйКлассификатор.Сокращение + "" "" + С_АдресныйКлассификатор.Наименование КАК Наименование,
       	|	С_АдресныйКлассификатор.ПризнакАктуальности КАК ПризнакАктуальности,
		...
		|	И Адреса.КодГородаВКоде = С_АдресныйКлассификатор.КодГородаВКоде
		|	И Адреса.КодНаселенногоПунктаВКоде = С_АдресныйКлассификатор.КодНаселенногоПунктаВКоде
		|	И Адреса.КодУлицыВКоде = С_АдресныйКлассификатор.КодУлицыВКоде
		|ГДЕ
		|	С_АдресныйКлассификатор.ТипАдресногоЭлемента = 5
		|");
...
Функция ПолучитьАдресныйЭлемент(Знач НазваниеЭлемента, ТипЭлемента, ЭлементРодитель)
	...
	Если ЭлементРодитель.Код > 0 Тогда // проверка на соответствие подчинению родителю
		...
		Если ЭлементРодитель.ТипАдресногоЭлемента <= 5 Тогда
			...
			Если ЭлементРодитель.КодНаселенногоПунктаВКоде <> 0 Тогда
				ОграничениеПоКоду = ОграничениеПоКоду + Символы.ПС 
				+ "  И (С_АдресныйКлассификатор.КодНаселенногоПунктаВКоде = &КодНаселенногоПунктаВКоде)";
			...
			Если ЭлементРодитель.КодУлицыВКоде <> 0 Тогда
				ОграничениеПоКоду = ОграничениеПоКоду + Символы.ПС 
				+ "  И (С_АдресныйКлассификатор.КодУлицыВКоде = &КодУлицыВКоде)";
	...
	Если АдресноеСокращение <> "" Тогда
		ОграничениеПоКоду = ОграничениеПоКоду + Символы.ПС + "  И (С_АдресныйКлассификатор.Сокращение = &АдресноеСокращение)";
...
Функция ПолучитьРезультатЗапросаАвтоподбораДляРегистра(Знач Текст, СтруктураПараметров, КоличествоЭлементов)
	...
	Объект = Метаданные.РегистрыСведений.С_АдресныйКлассификатор;
	...
	Запрос.Текст = СтрокаПолей + " ИЗ РегистрСведений.С_АдресныйКлассификатор КАК ТаблицаРегистра ГДЕ ";
...
Процедура ФоновоеЗаданиеОчисткиКлассификатораАдресов(Знач Параметры, Знач АдресРезультата) Экспорт
	...
	Пока Позиция < КоличествоУдаляемых Цикл
		...
		//	КодРегионаСтрокой, РегистрыСведений.АдресныйКлассификатор_Локализация.НаименованиеРегионаПоКоду(КодРегиона),
...
Функция ЗапросДесериализацияАдресаПоПредставлениюКЛАДР(Знач ИсточникСтранМира = Неопределено) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|		ДанныеАдреса.Длина        КАК Длина,
		|
		|		МИНИМУМ(С_АдресныйКлассификатор.ТипАдресногоЭлемента) КАК УровеньПоКлассификатору,
		|		МИНИМУМ(С_АдресныеСокращения.Уровень)                 КАК УровеньПоСокращениям,
		|	
		...
		|		РегистрСведений.С_АдресныйКлассификатор КАК С_АдресныйКлассификатор
		|	ПО
		|		С_АдресныйКлассификатор.Наименование = ДанныеАдреса.Наименование
		|		И С_АдресныйКлассификатор.Сокращение = ДанныеАдреса.Сокращение
		|		И С_АдресныйКлассификатор.ТипАдресногоЭлемента <= 5
		|		И С_АдресныйКлассификатор.ТипАдресногоЭлемента >= ДанныеАдреса.Позиция0
		|	ЛЕВОЕ СОЕДИНЕНИЕ
		|		РегистрСведений.С_АдресныеСокращения КАК С_АдресныеСокращения
		|	ПО
		|		С_АдресныеСокращения.Сокращение = ДанныеАдреса.Сокращение
		|		И С_АдресныеСокращения.Уровень <= 5
		|	ЛЕВОЕ СОЕДИНЕНИЕ
...
Функция ЗапросРезультатыАвтоПодбораНаселенногоПунктаКЛАДР(Знач ВыбиратьСтрок, Знач СкрыватьНеактуальныеАдреса) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК ЗагруженныеРегионыВнутр
		|ГДЕ
		|	ТипАдресногоЭлемента = 1
		|	И 1 В (
		|		               ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = ЗагруженныеРегионыВнутр.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 2
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = ЗагруженныеРегионыВнутр.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 3
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = ЗагруженныеРегионыВнутр.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 4
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = ЗагруженныеРегионыВнутр.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 5
		|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = ЗагруженныеРегионыВнутр.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 6
		|	)
		|ИНДЕКСИРОВАТЬ ПО
		...
		|	
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК ПодобныеАдреса
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ 
		|	РегистрСведений.С_АдресныйКлассификатор КАК Регионы
		|ПО
		...
		|	 
		|ЛЕВОЕ СОЕДИНЕНИЕ 
		|	РегистрСведений.С_АдресныйКлассификатор КАК Районы
		|ПО
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ 
		|	РегистрСведений.С_АдресныйКлассификатор КАК Города
		|ПО
		...
		|
		|ЛЕВОЕ СОЕДИНЕНИЕ 
		|	РегистрСведений.С_АдресныйКлассификатор КАК НаселенныеПункты
		|ПО
...
Функция ЗапросРезультатыАвтоПодбораУлицыКЛАДР(Знач ВыбиратьСтрок, Знач СкрыватьНеактуальныеАдреса) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|	РегистрСведений.С_АдресныйКлассификатор КАК НаселенныйПункт
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Улицы
		|ПО
...
Функция ЗапросНаселенныеПунктыПоПредставлениюКЛАДР(Знач ЧастиАдреса, Знач ВыбиратьСтрок, Знач СкрыватьНеактуальныеАдреса) Экспорт
	...
	Запрос.Текст = "
		...
		|	НаселенныеПункты.Наименование КАК НаселенныеПунктыНаименование
		|ИЗ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Адреса
		|	
		|ЛЕВОЕ СОЕДИНЕНИЕ 
		|	РегистрСведений.С_АдресныйКлассификатор КАК Регионы
		|ПО
...
Функция ЗапросСписокАнализаЭлементаАдресаКЛАДР(Знач ЧастиАдреса, Знач Уровень, Знач Наименование, Знач Сокращение, Знач ПоискПоПодобию, Знач ВыбиратьСтрок) Экспорт
	...
	Если Уровень = 1 Тогда
		Запрос.Текст = Запрос.Текст + "
			|ЛЕВОЕ СОЕДИНЕНИЕ
			|	РегистрСведений.С_АдресныйКлассификатор КАК РегионЗагружен
			|ПО
			|	РегионЗагружен.КодАдресногоОбъектаВКоде = Адреса.КодАдресногоОбъектаВКоде
			|	И РегионЗагружен.ТипАдресногоЭлемента = 1
			|	И 1 В (
			|		               ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = РегионЗагружен.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 2
			|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = РегионЗагружен.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 3
			|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = РегионЗагружен.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 4
			|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = РегионЗагружен.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 5
			|		ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ПЕРВЫЕ 1 1 ИЗ РегистрСведений.С_АдресныйКлассификатор ГДЕ КодАдресногоОбъектаВКоде = РегионЗагружен.КодАдресногоОбъектаВКоде И ТипАдресногоЭлемента = 6
			|   )
			|";
	КонецЕсли;
	Если Уровень > 1 Тогда
...
Функция ЗапросАдресаПоИндексуКЛАДР(Знач ВыбиратьСтрок, Знач СкрыватьНеактуальные) Экспорт
	...
	Запрос = Новый Запрос("
		...
		|
		|ИЗ	
		|	РегистрСведений.С_АдресныйКлассификатор КАК ПочтовыеИндексы
		|	
		|ЛЕВОЕ СОЕДИНЕНИЕ
		|	РегистрСведений.С_АдресныйКлассификатор КАК Улицы
		|ПО	
...
//
Функция ЗагрузитьАдресныеСведения_Локализация(КодАдресногоОбъекта, ПутьКДаннымНаСервере, АдресныеСведения, АльтернативныеНазвания, Знач ТипАдресногоЭлемента=Неопределено)
	...
	Если АдресныеСведения.Колонки.Найти("SOATO") = Неопределено Тогда
	...
	Если НужнаИндексация Тогда
		...
		ФайлДанных.Индексы.Добавить("IDXCODE", "SOATO", Истина);  
	...
	Пока Не ФайлДанных.ВКонце() Цикл
		...
		SOATO = СокрЛП(Строка(Формат(ФайлДанных.SOATO,"ЧГ= -")));
		...
		Если Не ТипАдресногоЭлементаУстановлен Тогда
			...
			ДатаЛиквидации = Дата(ФайлДанных.DATE_DEL);
			Если ЗначениеЗаполнено(ДатаЛиквидации) Тогда
				ФайлДанных.Следующая();
				Продолжить;
			КонецЕсли;  
			
			// Отсекаем устаревшие коды SОАТО
			
			Если СтрДлина(СокрЛП(SOATO))<10 Тогда
				ФайлДанных.Следующая();
				Продолжить;
			КонецЕсли; 
			
			
			Если СокрЛП(ФайлДанных.SHORTNAME) = "с/с" или СокрЛП(ФайлДанных.SHORTNAME) = "р-н" или СокрЛП(ФайлДанных.SHORTNAME) = "обл." Тогда
				ФайлДанных.Следующая();
				Продолжить;
			КонецЕсли;
			
			
			Код = SOATO + "0000000000";
			
			
			Если СокрЛП(ФайлДанных.NAME_ATE) = "Боровуха" или СокрЛП(ФайлДанных.NAME_ATE) = "Боровуха-1" или СокрЛП(ФайлДанных.NAME_ATE) = "Междуречье" или  СокрЛП(ФайлДанных.NAME_ATE) =  "Боровухский поселковый Совет" Тогда
			...
			Если Контроль[Код] = НеОпределено Тогда
                ...
                // Создаем область
                Если Контроль[КодАдресногоОбъекта] = Неопределено Тогда
                    
                    Контроль[КодАдресногоОбъекта] = 0;
                ...
                КонецЕсли; 
                 ...
                 // Создаем район
                НаименованиеРайона = СокрЛП(ФайлДанных.DISTRICT);
				...
				Если СокрЛП(ФайлДанных.SHORTNAME) = "г." Тогда
					Если Сред(СокрЛП(ФайлДанных.NAME_ATE),1,3) = Сред(СокрЛП(ФайлДанных.DISTRICT),1,3)  Тогда
				        // Это город в качестве района
				        Сокращение   = "";
				        ЕстьРайон = Ложь;
					КонецЕсли;	
                ...
                Если Контроль[ПолныйКодРайона] = Неопределено Тогда
                    
                    Если ЕстьРайон Тогда
                        Контроль[ПолныйКодРайона] = 0;
						...
						если НаименованиеРайона = "Полоцкий" Тогда
                ...
                Если Контроль[ПолныйКодНаселенногоПункта] = Неопределено Тогда
                    
                    Контроль[ПолныйКодНаселенногоПункта] = 0;
					...
					ТипНаселенногоПункта = НРег(СокрЛП(ФайлДанных.SHORTNAME));
                    ...
                    // SОАТО запоминаем только для Населенных пунктов, что бы потом их можно было найти для загрузки улиц
                    НоваяСтрока.SOATO       = SOATO;
                   
                    НоваяСтрока.Наименование = СокрЛП(ФайлДанных.NAME_ATE);
		...
		ДатаЛиквидации = ФайлДанных.DATE_DEL_S;
		
		если Не ПустаяСтрока(ДатаЛиквидации) Тогда
			
			//ДатаЛиквидации = Дата(СокрЛП(ДатаЛиквидации));
		...
		Код = Число(SOATO)*10000000000 + Число(ФайлДанных.ID_STR)*1000000;
		
		СтрокаНаселенногоПункта = АдресныеСведения.Найти(SOATO, "SOATO");
		...
		Если СтрокаНаселенногоПункта<>Неопределено Тогда
			Если Контроль[Код] = Неопределено Тогда
				...
				Контроль[Код] = 0;
				НоваяСтрока = АдресныеСведения.Добавить();
				НоваяСтрока.Код = СтрокаНаселенногоПункта.Код + Число(ФайлДанных.ID_STR)*1000000;
				...
				НоваяСтрока.КодУлицыВКоде             = Число(ФайлДанных.ID_STR);
				НоваяСтрока.ПризнакАктуальности       = 0;
				НоваяСтрока.SOATO                     = SOATO;				
				
				Если СокрЛП(ФайлДанных.TYPE_STR) = "Улица" Тогда
					СокрНазвание = "ул.";
				ИначеЕсли СокрЛП(ФайлДанных.TYPE_STR) = "Переулок" Тогда
					СокрНазвание = "пер.";
				ИначеЕсли СокрЛП(ФайлДанных.TYPE_STR) = "Проезд" Тогда
					СокрНазвание = "пр-д";
				ИначеЕсли СокрЛП(ФайлДанных.TYPE_STR) = "Проспект" Тогда
					СокрНазвание = "пр-т";
				Иначе
					СокрНазвание = СокрЛП(ФайлДанных.TYPE_STR);
				КонецЕсли;
				
				
				НоваяСтрока.Наименование = СокрЛП(ФайлДанных.NAME_STR);
				
				//Если  НРЕГ(СокрЛП(ФайлДанных.TYPE_STR))
				
				НоваяСтрока.Сокращение   = НРЕГ(СокрЛП(СокрНазвание));
...
Функция ПолучитьАдресныйклассификатор(МассивРегионов, ЗаписьТекст, КодОШибки,	Отказ) Экспорт
	
	МассивРегионов = СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(МассивРегионов, ",");
	
	//тест = Справочники.ОбновлениеАдресногоКлассификатора.НайтиПоКоду("000000002");
	//
	//ЗаписьТекст = ЗаписьТекст +  тест.Текст0;
	//
	//Для каждого ЭлМассива из МассивРегионов Цикл 
	//	
	//	ЗаписьТекст = ЗаписьТекст + тест["Текст" + Строка(ЭлМассива)];
	//	
	//КонецЦикла;	
	//
	//
	
	ЗаписьТекст = ЗаписьТекст +  Справочники.С_ОбновлениеАдресногоКлассификатора.АдресныйКлассификатор.Текст0;
	
	Для каждого ЭлМассива из МассивРегионов Цикл 
		
		ЗаписьТекст = ЗаписьТекст + Справочники.С_ОбновлениеАдресногоКлассификатора.АдресныйКлассификатор["Текст" + Строка(ЭлМассива)];
		
	КонецЦикла;	
	
	ЗаписьТекст = ЗаписьТекст + "</Items>";
