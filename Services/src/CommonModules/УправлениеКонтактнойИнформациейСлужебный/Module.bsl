...
Функция ТипВидаКонтактнойИнформации(Знач ВидИнформации) Экспорт
	...
	Если ВидИнформации.Наименование = "Телефон" тогда
		Результат = Перечисления.ТипыКонтактнойИнформации.Телефон;
	ИначеЕсли ВидИнформации.Наименование = "Электронная почта" тогда
...
Функция ИсключатьПроверкуДомаВАдресе(Знач АдресРФ)
	...
	Если ВРег(СокрЛП(АдресРФ.СубъектРФ)) = НСтр("ru = 'САМАРСКАЯ ОБЛ'") И ВРег(СокрЛП(АдресРФ.Город)) = НСтр("ru = 'САМАРА Г'") Тогда
		Результат = Истина;
	КонецЕсли;
