...
Процедура ОтчетСкомпоноватьРезультат(Команда)
	ЗапускЗаданияНаСервере();
	ОчиститьСообщения();
	Сформировать();
КонецПроцедуры

&НаСервере
Процедура ЗапускЗаданияНаСервере()
	...
	// Запрос без отбора для удаления из регистра сведений РегистрацияДействийПользователей данных созданных ранее текущего месяца 
