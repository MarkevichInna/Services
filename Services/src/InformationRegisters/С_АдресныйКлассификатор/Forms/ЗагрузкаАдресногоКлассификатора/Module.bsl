...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	//Если АдресныйКлассификаторКлиентСервер_Локализация.ЭтоWindowsКлиент() Тогда
	...
	Если ТекущийКодРегиона = Неопределено И Параметры.Свойство("НазваниеРегионаДляЗагрузки", НазваниеРегионаДляЗагрузки) Тогда;
		...
		ТекущийКодРегиона = РегистрыСведений.С_АдресныйКлассификатор.КодРегионаПоНаименованию(НазваниеРегионаДляЗагрузки);
	КонецЕсли;
...
Процедура АдресЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	С_АдресныйКлассификаторКлиент.ВыбратьКаталог(ЭтотОбъект, "АдресЗагрузки", 
...
Процедура ЗагрузитьКлассификаторИзКаталога(Знач КаталогДанных)
	...
	РезультатАнализа = С_АдресныйКлассификаторКлиентСервер.АнализДоступностиФайловКлассификатораВКаталоге(КодыРегионов, КаталогДанных, ПараметрыЗагрузки);
...
Процедура ЗагрузитьКлассификаторИзКаталогаИТС(Знач КорневойКаталог)
	...
	Для Каждого Вариант Из ВариантыКаталогов Цикл
		РезультатАнализа = С_АдресныйКлассификаторКлиентСервер.АнализДоступностиФайловКлассификатораВКаталоге(КодыРегионов, Вариант, ПараметрыЗагрузки);
...
Процедура ЗагрузитьКлассификаторССайта(Знач Аутентификация = Неопределено)
	...
	Если ПустаяСтрока(Аутентификация.Логин) Тогда
		...
		ОткрытьФорму("РегистрСведений.С_АдресныйКлассификатор.Форма.АвторизацияНаПользовательскомСайте", , ЭтотОбъект, , , ,Оповещение);
		Возврат;
	КонецЕсли;
...
Процедура ЗагрузитьКлассификаторССайтаКЛАДР()
	...
	КаталогЗагрузки = С_АдресныйКлассификаторКлиент.ВременныйКаталогКлиента();
	УдалитьПослеПередачиНаСервер = Новый Массив;
	УдалитьПослеПередачиНаСервер.Добавить(КаталогЗагрузки);
	
	Результат = С_АдресныйКлассификаторКлиент.ЗагрузитьКЛАДРСВебСервера("SO", Аутентификация, КаталогЗагрузки);
	Если Не Результат.Статус Тогда
		...
		С_АдресныйКлассификаторКлиентСервер.УдалитьВременныйФайл(КаталогЗагрузки);
	...
	Для Каждого КодРегиона Из КодыРегионов Цикл
		...
		Результат = С_АдресныйКлассификаторКлиент.ЗагрузитьКЛАДРСВебСервера(ОбъектЗагрузки, Аутентификация, КаталогЗагрузки);
...
Процедура ЗапуститьФоновуюЗагрузкуИзКаталогаКлиентаПродолжение()
	...
	Для Каждого ИмяФайла Из ПараметрыЗагрузки.УдалитьПослеПередачиНаСервер Цикл
		С_АдресныйКлассификаторКлиентСервер.УдалитьВременныйФайл(ИмяФайла);
	КонецЦикла;
...
Процедура ЗапуститьФоновуюЗагрузкуНаСервере(Знач КодыРегионов, Знач ОписаниеФайловЗагрузки, Знач Режим = Неопределено)
	...
	Попытка
		...
		//	"АдресныйКлассификатор_Локализация.ФоновоеЗаданиеЗагрузкиКлассификатораАдресов",
		//	ПараметрыМетода,
		//	НСтр("ru = 'Загрузка адресного классификатора'")
		//);
	С_АдресныйКлассификаторСервер.ФоновоеЗаданиеЗагрузкиКлассификатораАдресов(ПараметрыМетода,"С_АдресныйКлассификатор.ФоновоеЗаданиеЗагрузкиКлассификатораАдресов");
...
Функция РаспаковатьАрхивИТС(Знач ПолноеИмяФайла, Знач МаскаОжидаемого)
	...
	КаталогКлиента = С_АдресныйКлассификаторКлиент.ВременныйКаталогКлиента();
	...
	Найденное = С_АдресныйКлассификаторКлиентСервер.НайтиФайл(КаталогКлиента, МаскаОжидаемого);
...
Функция СохраненныеДанныеАутентификацииСайта()
	...
	Возврат С_АдресныйКлассификаторСервер.ПараметрыАутентификацииНаСайте();
...
//удаляем не нужные строки перед закрытием
...
Процедура ЗакрытьФормуНаСервере()
	...
	Попытка
		...
		Если ТаблицаОставляемыхЗаписей.Найти(50000000010000000000, "Код") <> Неопределено Тогда
			...
			ТаблицаОставляемыхЗаписей.Найти(50000000010000000000, "Код").Сокращение = "г.";
