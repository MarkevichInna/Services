...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ВидКонтактнойИнформации.Тип = Перечисления.ТипыКонтактнойИнформации.Адрес Тогда
		...
		Если РедактированиеТолькоВДиалоге  Тогда
			УстановитьОформлениеПредставленияДляРедактированияТолькоВДиалоге();
		КонецЕсли;
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗакрытиеФормыБезСохранения Или ЗавершениеРаботы Тогда
		Возврат;
	КонецЕсли;
...
Процедура ИсторияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Элемент.ТекущийЭлемент.Имя = "ИсторияПредставление" 
...
Процедура ОК(Команда)
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		Отбор = Новый Структура("ДействуетС", История[Индекс].ДействуетС);
...
&НаСервере
Процедура УстановитьОформлениеПредставленияДляРедактированияТолькоВДиалоге()
	
	Элементы.ИсторияПредставление.РедактированиеТекста = Ложь;
	Элементы.ИсторияПредставление.ГиперссылкаЯчейки = Истина;
	ЦветГиперссылки = Новый Цвет(28, 85, 174);
	
	Элемент = УсловноеОформление.Элементы.Добавить();
	ПолеЭлемента = Элемент.Поля.Элементы.Добавить();
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Элементы.ИсторияПредставление.Имя);
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("История.Представление");
	ОтборЭлемента.ВидСравнения = ВидСравненияКомпоновкиДанных.Заполнено;
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветГиперссылки);
	...
	Элемент.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветГиперссылки);
	Элемент.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", УправлениеКонтактнойИнформациейКлиентСервер.ТекстПустогоАдресаВВидеГиперссылки());
...
Функция КонтактнаяИнформацияНеКорректная()
	...
	Для Индекс = 0 По История.Количество() - 1 Цикл
		...
		ИначеЕсли НЕ СтрокаИстории.ЭтоИсторическаяКонтактнаяИнформация Тогда
