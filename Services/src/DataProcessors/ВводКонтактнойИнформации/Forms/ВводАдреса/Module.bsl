...
Процедура ОтобразитьИнформациюОДатахДействияАдреса(ДействуетС, ДействуетДо)
	...
	Иначе
		ТекстИсторическийАдрес = НСтр("ru = 'Адрес действует в настоящее время'");
	КонецЕсли;
...
Функция КонтекстФормыКлиент(НовыеДанные = Неопределено)
	...
	СписокРеквизитов = "
		...
		|Модифицированность, ЕстьКлассификатор, ДополнительныйЭлемент, ПодчиненныйЭлемент, ЭтоВебСервис
		|";
